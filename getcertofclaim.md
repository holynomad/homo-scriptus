# HIS 진료개발자의 보험심사평가사 2급 취득기 (**feat. `40대 개획자 아저씨`**)

## 배경

  - 왜, 갑자기 ?
    - 10여년 넘게 병원 전산시스템 **개발자**로 살아오다가 **기획자**로 전향하면서 조금 더 넓은 스콥의 전문성 심화니즈 발생
    - 더 늦기 전에 (도메인날리지) 하나라도 더 배워보자
    - 코로나 팬더믹 어드밴티지
  - 보험심사평가사 2급 ? (아래 내용 2021.7. 기준)
    - 자세한 건 요기 👉 [한국의료행정실무협회 보험심사평가사 2급](https://www.simsa.kr/03_information/information_02.php)
    - <img width="698" alt="스크린샷 2021-10-11 오후 11 37 46" src="https://user-images.githubusercontent.com/43846264/136809378-e65b7dad-e458-44d9-9e7f-4123e794386e.png">
    - 한줄요약 : `민간자격`, 2급 응시제한 요건 ✖️
    - 시험과목 : 요양급여기준 1️⃣,2️⃣ (30문항, 객관식), 의료기초(의료법, 의학용어) (40문항, 객관식), 외래실습 (5문항, 주관식) 
    - 합격기준 
      - 필기 : 과목별 40점 이상
      - 실기 : 과목별 60점 이상
      - 전 과목 평균 60점 이상
  
## 준비

  - 제출서류 
    - 수험원서 원본 1️⃣부 (온라인 신청시 반명함판 사진, 경력증명자료 (해당시))
    - 의료기초(의료법, 의학용어) 면제대상자는 아니라서 pass... 👉 [시험과목 일부면제자 조건체크](https://www.simsa.kr/03_information/information_02.php)
  - [2021년 하반기 시험일정](https://www.simsa.kr/02_notice/notice_01.php?md=view&no=282&page=1&refer=)
    - 시험일 : 11/21(일) **14:00 ~ 16:30** (코로나19 감염확산방지 위한 온라인시험 예정) ➡️ [시행공고 바로가기](https://www.simsa.kr/02_notice/notice_01.php?md=view&no=290&page=1&refer=%3Ftb%3Din_bbs_notice%26page%3D1)
    - 접수기간 : 9/27(월)~11/5(금)
  - [원서접수](https://www.simsa.kr/04_receipt/receipt_01.php)
    - 인터넷접수 준비물 (관련문의 T.02-6342-0234) : 한국의료행정실무협회 회원가입, 반명함판 스캔이미지, 검정수수료 결제(2급 : `46,000원`, 이*페이 PG연동되어 현X카드 앱카드로 결제완료 ☑️), 인터넷 원서작성
  - 기타 `부대비용` : `17,000원` + @

  - 합격수기 리뷰

## 착수

  - 예상문제집 최신판(개정13판) 구매 : `19,800원`
  - 팁 
    - K병원 심사팀 제니퍼샘
    - who else ?

  - 관련 유튜브 채널
    - [(사)한국의료행정실무협회 온라인 시험방법 및 절차 (www.simsa.kr, 2020.6.)](https://youtu.be/24sguWhTK6A)
    - [건강보험 청구심사 실무특강 샘플 강의 (너스케입, 2020.7.)](https://youtu.be/7LpKU2UYSAU)
    - [보험심사, 의학용어 샘플강의 (너스케입, 2018.9.)](https://www.youtube.com/watch?v=4G0NetkSXtI&list=PLUihElL8hIZszif6gLbnh9TGcj43YbmjT)
    - [[보험심사] 급여, 비급여, 100:100 용어 / 슬기로운 심사공부 (양쌤튜브, 2020.5.)](https://youtu.be/y0dFhjSKpGw)
    - [심사청구가 뭐지!? (심청이TV, 2020.1.)](https://youtu.be/6OJSp-hEAHk)
    - [건강보험심사평가원의 진료비 심사/평가 과정 (HIRA, 2018.4.)](https://youtu.be/C9ok-ypD2sQ)
    - [요양급여 심사청구 강좌 (부산디지털대학교)](http://www.kocw.net/home/cview.do?cid=a8b0e30c5047185f)

## 진행

  - 제1부 요양급여기준
    - `문제접근 플로우`
      - 예시1) `(64p) 고열구역관류요법 시행시 적정 행위료 산정으로 옳은 것` ?
        - [심퍙원 > 제도정책 > 보험인정기준](https://www.hira.or.kr/rd/insuadtcrtr/InsuAdtCrtrList.do?pgmid=HIRAA030069000000) > "고열구역관류요법" 키워드 검색
        - <img width="1035" alt="스크린샷 2021-09-13 오전 1 04 11" src="https://user-images.githubusercontent.com/43846264/132994725-b628b228-72d7-4d22-a801-70f61be3562e.png">
        - 고시내용 중 "산정기준" 부분 체크 (👉 자 190 부분체외순환으로 산정)
        - <img width="1396" alt="스크린샷 2021-09-13 오전 1 05 23" src="https://user-images.githubusercontent.com/43846264/132994755-b1ba1e11-de12-47ec-941d-6212e80a635e.png">
        - 더욱 상세한 내역을 보고 싶을 땐, [건강보험심사평가원 청구방법 및 급여기준 조회시스템](http://rulesvc.hira.or.kr/lmxsrv/law/lawFullView.srv?SEQ=50) > "자 190" 검색 
        - <img width="1440" alt="스크린샷 2021-09-13 오전 1 10 58" src="https://user-images.githubusercontent.com/43846264/132994930-e6c3cd8b-ac99-4764-a125-9d0733ff4573.png">


      - 예시2) `전자궁적출술 후 1주일 후에 출혈이 심하여 개복하여 지혈 시 적정 수기료 산정으로 옳은 것` ?
        - [건강보험심사평가원 청구방법 및 급여기준 조회시스템](http://rulesvc.hira.or.kr/lmxsrv/law/lawFullView.srv?SEQ=50) > "전자궁적출술" 검색 (👉 자 244 산정)
        - <img width="1437" alt="스크린샷 2021-09-13 오전 1 19 13" src="https://user-images.githubusercontent.com/43846264/132995139-e179ab8a-932f-43c0-9db3-f9f9c78d3d32.png">

      - 예시3) `방사선 단순영상 진단 시 동일부위에 대해 산정 가능한 최대 매수로 옳은 것` ?
        - 구글링 "건강보험 방사선단순영상진단" 검색 (👉 심평원 청구방법 및 급여기준 조회시스템 링크클릭)
        - <img width="858" alt="스크린샷 2021-09-14 오전 1 05 51" src="https://user-images.githubusercontent.com/43846264/133118698-1e008644-da97-4824-8fdd-e00a7a0af706.png">
        - [건강보험심사평가원 청구방법 및 급여기준 조회시스템](http://rulesvc.hira.or.kr/lmxsrv/law/lawFullView.srv?SEQ=50) > "방사선단순영상진단" 검색 > 👉 요양급여기준 제3장 제1절 주3 참조시 `최대5매` 산정
        - <img width="1424" alt="스크린샷 2021-09-14 오전 1 13 04" src="https://user-images.githubusercontent.com/43846264/133119770-8434caa8-6eb3-442b-aea5-fb2ce15bfb8a.png">
        - <img width="500" alt="스크린샷 2021-09-14 오전 1 14 17" src="https://user-images.githubusercontent.com/43846264/133119917-bbcdf134-aaef-4abc-a10e-a8852a780764.png">

      - 예시4) `항파킨슨 약제 투여의 요양급여 인정기준에 대한 설명으로 옳지 않은 것` ?
        - 구글링 "항파킨슨 약제 요양급여 인정기준" 검색 (👉 요양급여의 적용기준 및 방법에 관한 세부사항(약제).pdf 클릭)
        - <img width="727" alt="스크린샷 2021-09-15 오전 12 19 17" src="https://user-images.githubusercontent.com/43846264/133285852-b36bc11c-86c9-40ef-856f-6620127ee6d1.png">
        - 관련 인정기준 중 잘못언급된 부분 체크 🙂
        - <img width="444" alt="스크린샷 2021-09-15 오전 12 21 16" src="https://user-images.githubusercontent.com/43846264/133286215-5536ae08-47e9-47b2-8bde-29c9ac1fbf11.png">

      - 예시5) `골관절염 환자에게 Fentanyl 패취제 투여시 1회 최대처방 허용일수` ?
        - www.hira.or.kr ➡️ 제도/정책 ➡️ 보험인정기준 ➡️ 고시 ➡️ 검색창 `Fentanyl` ➡️ 2017-153호 마약성진통제 [일반원칙] 참조 ➡️ 페이지 내 `1회` 검색 ➡️ 1회처방당 최대 `30일`까지 인정
        - <img width="1030" alt="스크린샷 2021-11-19 오전 12 16 03" src="https://user-images.githubusercontent.com/43846264/142443600-2c635fa8-d6b1-481a-b3ef-a7bddbce84b3.png"><img width="668" alt="스크린샷 2021-11-19 오전 12 17 46" src="https://user-images.githubusercontent.com/43846264/142443924-ccae6b5c-d0ed-409a-a63b-1352d5f862bb.png"><img width="521" alt="스크린샷 2021-11-19 오전 12 18 38" src="https://user-images.githubusercontent.com/43846264/142444080-7393bca8-bf4c-46d1-a33e-2fb8a266a3e6.png">


  - 제2부 의료기초 
    - `제1장 의료관계법규 문제접근 플로우`
      - 예시1) 요양병원을 개설할 수 있는 자로만 옳게 조합된 것은 ?
        - 구글링 "의료법 요양기관 개설" 검색 (👉 의료법 제33조 클릭)
        - <img width="710" alt="스크린샷 2021-09-15 오전 12 24 53" src="https://user-images.githubusercontent.com/43846264/133286859-0367c205-b2e3-48d0-b79e-eda3e335cbee.png">
        - 요양병원 개설이 가능한 조합찾기 (👉 의사 & 한의사)
        - <img width="1383" alt="스크린샷 2021-09-15 오전 12 26 54" src="https://user-images.githubusercontent.com/43846264/133287228-a0099771-0d19-4b48-b184-16573ead8f23.png">
      - 예시2) 요양기관이 요양급여에 관한 서류를 보존하지 않았을 때의 과태료로 옳은 것은 ?
        - 구글링 "요양기관 서류 보존 과태료" 검색 (👉 국민건강보험법 제119조 과태료) 
        - <img width="872" alt="스크린샷 2021-11-20 오후 4 23 39" src="https://user-images.githubusercontent.com/43846264/142718413-7cc22e56-f90c-4e41-8eb8-fc3cf5494363.png">
        - 👉 과태료 검색후 서류보존 관련 부분 (100만원 blah blah..) 체크
        - <img width="1075" alt="스크린샷 2021-11-20 오후 4 27 55" src="https://user-images.githubusercontent.com/43846264/142718508-a926d954-b302-46bd-b799-93041ae33296.png">
      - 예시3) 의료급여 수급권자가 2,3차 의료급여기관을 직접 이용할 수 없는 것은 ?
        - 구글링 "의료급여 수급 기관 절차" 검색 (👉 의료급여법 시행규칙 )
        - <img width="853" alt="스크린샷 2021-11-20 오후 4 46 26" src="https://user-images.githubusercontent.com/43846264/142718941-56ad8069-3cba-4ead-9c97-fa3e0e059588.png">
        - 👉 제3조 의료급여의 절차
        - <img width="1434" alt="스크린샷 2021-11-20 오후 4 52 05" src="https://user-images.githubusercontent.com/43846264/142719072-accb2582-f561-47b4-86b8-9215a4882c94.png">
      - 예시4) 제1차 의료급여기관으로 옳지 않은 것은 ?
        - 👉 의료급여법 제9조 의료급여기관 검색
        - <img width="1439" alt="스크린샷 2021-11-20 오후 5 01 21" src="https://user-images.githubusercontent.com/43846264/142719282-69b50a1a-f335-4021-823d-a67819178a69.png">
        - 👉 `개설신고`와 `개설허가` 구분 체크
      - 예시5) 65세 이상인 건강보험 가입자의 의원 요양급여비용이 15,000원 이하인 경우 본인부담액으로 옳은것은 ?
        - 구글링 "65세 의원 건강보험 요양급여비용" 검색 (👉 HIRA 건강보험 본인부담기준 안내)
        - <img width="1032" alt="스크린샷 2021-11-20 오후 5 09 11" src="https://user-images.githubusercontent.com/43846264/142719477-729601a0-a0df-4ce3-8290-91bbea03e546.png">
        - 쭈~욱 아래로 내리다보면 `의원급` + `65세이상` 보임...👉 1,500원 !!
        - <img width="1038" alt="스크린샷 2021-11-20 오후 5 10 14" src="https://user-images.githubusercontent.com/43846264/142719489-777c8bbe-8092-490a-9bf2-4885280c7bcf.png">


    
    - `제2장 의학용어 문제접근 플로우` 
      - 예시1) 혈전의 섬유소를 용해시키는 분해요소는 ?
        - Plasmin : 혈전분해효소 ➡️ 그냥 머릿속에 강제로 집어넣는 것외엔 노답... 😧
      - 예시2) 서로 의미와 연결이 맞지 않는 것은 ?
        - `Staxis : 출혈`, `Pexy : 고정`, `Lysis : 분리`, `Stasis : 문합`
        - 구글링 "접미사 pexy" 검색 👉 Stasis는 `stop`의 의미 (문합은 Stomy)
        - 👉 **접두사 & 접미사** 기본규칙 외워야 하나 😥
        - <img width="850" alt="스크린샷 2021-11-20 오후 5 17 48" src="https://user-images.githubusercontent.com/43846264/142719669-84f27b87-5473-4d5a-99fe-f47489994e44.png">


  - 제3부 외래실습 (a.k.a. 외래 명세서 실습 : 외래 진료기준 보험심사 실습)

    - 참고 : [20년 전반기 2급 시험특강 (양쌤튜브)](https://www.youtube.com/watch?v=RuAabm_NzhE&list=PLZX9UECS4orhTwk3V_u3-nSz6FzGaVHa-&index=9)

    - `두 가지 factors`
      - 진단명에 맞는 처방인가 ?
      - 요양급여기준에 맞는 처방횟수 인가 ?
      - 예시) 진단명 : 우측 발목의 염좌
        - 검사료 : 발목 X-ray (3매) 1(횟수) x 1(일수), 발목 MRI 1 x 1
        - 주사료 : ketorolac (진통제) 1A(용량: 1 Ample) x IM(용법: 엉덩이주사), 근육주사 1 x 1 (IM 👉 행위료)
        - 이학요법료 : 요추부 전기자극치료 1 x 1
        - 👉 발목 MRI 삭제 (과다청구), 요추부 전기자극치료 삭제 (과다청구)

      - 응급의료관리료 ?
        - KTAS 1~5등급에 따라 응급증상(1~3등급) 또는 응급에 준하는 증상(4등급) 있어야 산정가능 (👉 5등급은 산정제외)
      - 수술후 처치료 (a.k.a. dressing) ?
        - 수술 후 익일부터 산정가능, 진단명 매칭필요
      - 수액제 주입로를 통한 주사료 (IVS = IV Side) ?
        - 외래는 1일 1회 산정, 입원은 1일 2회 산정가능
      - 처치 및 수술료의 야간가산 ?
        - <img width="540" alt="스크린샷 2021-10-27 오전 12 06 29" src="https://user-images.githubusercontent.com/43846264/138907960-512de790-289d-4d6c-8f93-e4deee8bf455.png"> (참조 : [닥터김의 건강보험 청진기](https://blog.naver.com/PostView.nhn?blogId=39954&logNo=221886305234&parentCategoryNo=&categoryNo=72&viewDate=&isShowPopularPosts=true&from=search))
        - 진단명과 매칭하여 `응급진료가 불가피한 경우`에 한한 야간가산 50% 산정가능
      - 화상처치 vs. 수술후 처치 동시산정 ?
        - 기본적으로 처치 및 수술시에 사용한 약제 및 치료재료는 별도산정 없음 👉 예외적으로 별도산정하는 케이스는 `처치 및 수술료 산정지침 11항` 참조
        - 수술후 처치는 수술당일 산정안됨 (익일부터 산정) 👉 `초진진찰료` 나왔는데 `수술후 처치` 끊어져있는건 무조건 삭감
      - 피부과 환자(L82 지루각화증) 주사료 산정기준 ?
        - 병변내 주입요법 & 피하근육내 주사 동시산정 ? : 스테로이드제제를 직접 환부에 주입([트리암시놀론주사의 병변내 주입요법 참조](https://m.blog.naver.com/kataba6/221621824940)) 👉 피하근육내 주사(수기료)와 동시산정 안됨 (수기료 우선순위 앞선다 ??)
      - 단순처치(Simple Dressing) vs. 염증성처치(Infectious Wound Dressing) ?
        - 처치관련 [상대가치점수 행위정의 정리 참조](https://m.blog.naver.com/39954/221880553849)
      - (의원급) 외래환자 의약품관리료 산정기준 ?
        - 외용약 or 주사제 단독 조제 경우 `1일분 소정점수` 산정
        - ```
          의원, 치과의원, 보건의료원 의·치과 

          ㈎ 내복약 조제일수에 따라 산정한다. 

          ㈏ 외용약 또는 주사제를 내복약과 복합으로 조제한 경우에는 내복약의 조제일수에 의한다. 

          ㈐ 위 “㈎”의 규정에도 불구하고 외용약 및 주사제(약가를 산정하지만 주사료 등이 발생하지 아니하는 경우를 포함)를 복합 또는 단독으로 조제한 경우에는 1일분 소정점수를 산정한다. 
          ```
      - (병원급 이상) 외래환자 의약품관리료 산정기준 ?
        - 병원급 이상은 `방문당`으로 산정
        - ```
           상급종합병원, 종합병원, 병원, 치과병원, 요양병원·한방병원 내 의·치과 

           ㈎ 외래환자에게 투약한 경우 방문당으로 산정한다.

           ㈏ 외용약 및 주사제(약가를 산정하지만 주사료 등이 발생하지 아니하는 경우를 포함)를 복합 또는 단독으로 조제한 경우에도 외래환자 의약품 관리료[방문당] 소정점수를 산정한다. 
           ```
      - LDH, CPK, CK-MB 검사 정리
        - LDH : 간기능 검사
        - <img width="854" alt="스크린샷 2021-11-20 오후 6 04 10" src="https://user-images.githubusercontent.com/43846264/142720876-eebab27c-0ea7-4bae-bc37-394f078195a9.png">
        - CPK : 심근경색 검사 
        - <img width="847" alt="스크린샷 2021-11-20 오후 6 05 15" src="https://user-images.githubusercontent.com/43846264/142720901-dfcfee4e-15bc-4714-b43a-7a62e903bc42.png">
        - CK-MB : 심근경색 검사
        - <img width="859" alt="스크린샷 2021-11-20 오후 6 06 08" src="https://user-images.githubusercontent.com/43846264/142720919-3d271037-f6a6-4bed-b815-86d899f60ff4.png">
      - Color Print Paper 전규격 및 결장경검사 요양급여기준 ?
        - 아무리 colonocopy와 color print paper 보험급여로 검색해봐도...상관관계를 모르겠다만... 내시경 검사결과 출력과 관련해서 급여적용이 가능한듯 (아래는 안저검사 촬영에 대한 Color Print Paper 요양급여 인정기준 설명)
        - <img width="848" alt="스크린샷 2021-11-20 오후 6 17 51" src="https://user-images.githubusercontent.com/43846264/142721212-63a07a49-42db-4efd-9572-b913b1c98265.png">
      - 거즈 or 붕대 요양급여 기준 ?
        - 화상처치 경우만 인정 (피부과처치, 단순처치 등 해당없음)
      - 외래환자 조제/복약지도료 정리
        - 의약분업 예외사유 환자에게 당해 의료기관 약사가 조제실에서 조제투약한 경우 산정가능
        - 내복약은 기본 `1회당` 산정인데, [약제 급여목록 및 급여 상한금액표] 고시에서 정한 상한금액이 `포장단위로 책정된 의약품(병, 팩 등)`을 지급하는 경우 `1일분`으로 산정 (예: 아가메이트젤리 포장단위 25g/플라스틱컵 x 21개)
      - 계속적복막관류술
        - 처치 및 수술료 1일당 산정
      - 아포지단백 A,B 검사 
        - 심장질환, 고지혈증 과거력/가족력 있는 환자의 심혈관계질환 위험도 평가시 실시 (cf. 고혈압성신장질환 X)
      - 혈액투석 및 재료대 정리
        - 1회당 산정 (초재진 진찰료 체크할 것)
      - 





 
 
## 온라인 시험 진행방법

  - <img width="515" alt="스크린샷 2021-09-28 오전 12 30 21" src="https://user-images.githubusercontent.com/43846264/134939424-7554d917-d8e9-40aa-a410-72962c41f2d7.png">
  - <img width="506" alt="스크린샷 2021-09-28 오전 12 31 21" src="https://user-images.githubusercontent.com/43846264/134939579-0eadc864-8401-4b81-93be-3c637ae0a95a.png">
  - <img width="515" alt="스크린샷 2021-09-28 오전 12 31 43" src="https://user-images.githubusercontent.com/43846264/134939627-443502b2-d88b-4a56-95fc-59272f678b2e.png">
  - <img width="508" alt="스크린샷 2021-09-28 오전 12 32 14" src="https://user-images.githubusercontent.com/43846264/134939696-e4d1d26f-3fd8-42bb-96e5-a0cb92578c97.png">
  - <img width="512" alt="스크린샷 2021-09-28 오전 12 32 36" src="https://user-images.githubusercontent.com/43846264/134939765-c2d905b6-aea4-4e78-bb6f-9a869f47ceae.png">
  - <img width="514" alt="스크린샷 2021-09-28 오전 12 33 03" src="https://user-images.githubusercontent.com/43846264/134939839-37358879-2846-4cc0-84df-28d5f08a38fe.png">
  - <img width="510" alt="스크린샷 2021-09-28 오전 12 33 51" src="https://user-images.githubusercontent.com/43846264/134939954-c9eb7987-598b-434d-83ad-ca383cc9919e.png">
  - <img width="507" alt="스크린샷 2021-09-28 오전 12 34 20" src="https://user-images.githubusercontent.com/43846264/134940022-10a5e367-ae65-4d52-9525-2834d8091597.png">
  - <img width="509" alt="스크린샷 2021-09-28 오전 12 34 45" src="https://user-images.githubusercontent.com/43846264/134940087-9395fe01-7168-4332-91f1-92b1e26f67c3.png">
  - <img width="511" alt="스크린샷 2021-09-28 오전 12 35 03" src="https://user-images.githubusercontent.com/43846264/134940134-3db7a4a1-acfe-4d90-8281-7b095aee9620.png">
  - <img width="508" alt="스크린샷 2021-09-28 오전 12 35 24" src="https://user-images.githubusercontent.com/43846264/134940185-fb7e65f4-bed4-493c-a6c5-83a9d9beb944.png">
  - <img width="512" alt="스크린샷 2021-09-28 오전 12 35 52" src="https://user-images.githubusercontent.com/43846264/134940257-17a402ef-3b9d-4532-97a0-738137a450db.png">
  - <img width="511" alt="스크린샷 2021-09-28 오전 12 36 22" src="https://user-images.githubusercontent.com/43846264/134940357-438366ed-ff9f-4b66-8ca3-a0cb8b5af828.png">
  - <img width="507" alt="스크린샷 2021-09-28 오전 12 37 25" src="https://user-images.githubusercontent.com/43846264/134940522-b4a662c0-1798-4879-b3a1-92aa188b3f93.png">
  - <img width="760" alt="스크린샷 2021-09-28 오전 12 39 36" src="https://user-images.githubusercontent.com/43846264/134940863-92928e7c-e96a-41a2-83c4-bd35b1f9a94d.png">



## D-day 준비

  - 일단 공부하나도 못했지만 13년 병원짬밥으로 일단 접수 @ 9/28(화) 00:21
  - 10월 중 문제풀이 과정 오픈참조 (심사콕 👇👇)
  - ```
    코로나19 확산방지와 수험자의 안전을 위하여 2021년도 제36회 보험심사평가사2급 자격시험 문제풀이 특강이 비대면(온라인)으로 유튜브 채널 "심사콕"을 통하여 무료로 진행됩니다.

    10월 중 매주 1~2강이 업로드 될 예정입니다.
    ※ 강의내용에 따라 강의 수는 변동될 수 있습니다.
    ※ 문제풀이 동영상은 11월 21일(일) 오전 11시에 삭제 됩니다. 

    아래 주소를 복사하여 인터넷 주소창에 붙여넣기 하면 확인하실 수 있습니다. 
    온라인 시험 응시방법 : https://youtu.be/24sguWhTK6A
    제36회 보험심사평가사 자격시험 안내 : https://youtu.be/famtexj5RzY

    제1강 의학용어 : https://youtu.be/K98yZQWeNt4
    제2강 의학용어 : https://youtu.be/1_LobKZjrG8
    제3강 의학용어 : https://youtu.be/tegMAZv0Vng
    제4강 의료법규 : https://youtu.be/mI-S9O1MV3w
    제5강 의료법규 : https://youtu.be/iwW2mCTOkNI
    제6강 요양급여기준1 : https://youtu.be/TAuX-3hrRyA
    제7강 요양급여기준1 : https://youtu.be/VN5SyzqafW0
    제8강 요양급여기준1 : https://youtu.be/s0yZ8K7EL3U
    제9강 요양급여기준2 : https://youtu.be/MlMtkxMqWfM
    제10강 요양급여기준2 : https://youtu.be/0y3i0OdZezE
    제11강 외래실습 : https://youtu.be/fASA0ezD55E
    ```

  - 각 카테고리별 키워드 요약 (구글 스프레드시트)
  - 외래실습 (외래상병 및 처방내역에 대한 삭감방지 심사), 어떻게 공부해야할까 ?
  - 요양급여기준 (산정기준 및 급여인정기준), 어떻게 공부해야할까 ?
  - `D-2w` 
    - 아직 열심히 공부하지 못하고 있음
    - 보험심사 업무를 하고 있는 지인에게 치트키를 받고 싶은 마음 간절함
    - 그래도 도메인 날리지 관점에서 어프로치 방법론은 습득하고 
    - 심사콕이랑 양쌤튜브(?) 보면서 감이라도 잡고가기
  - `D-1w` 
    - 일단 출퇴근길 심사콕 문제풀이특강 반복청취
  - `D-5`
    - 요양급여기준 `1` 훑는중... 그래도 이렇게라도 훑으면서 도메인 공부하니 좋타..회사에서도 도메인 스터디 개설해야지 📝
  - `D-4`
    - 요양급여기준 `2` 훑는중... 보험인정기준 정리하고, 찾아내는 기술은 CDSS 어드민 개발에 큰 도움이 될 것이다 🚡
  - `D-3`
    - 요양급여기준 `1`+`2` 반복... 또 반복... 
  - `D-2`
    - 요양급여기준 `1`+`2` 반복... 또 반복...하면서 느낀점 하나 : `이걸 외워서 시험보는 건 좀 아닌 것 같다. 어떻게 심사기준과 고시변경 Scoping & Depthening을 조금 더 빠르고 쉽게 할 수 있는지에 대한 방법론만 알려주면 될 것을`  
  - `D-1`
    - 의료법규 및 의학용어도 한번 쭉 훑었다. 어떻게 관련 법령을 찾아가고, 어떻게 이것을 CDSS에 적용할 수 있을지에 대한 관점(view)으로 공부했다. 준비는 끝났다. 
  - `D-day`
    - 막판 1주일 벼락치기로 최선(?)을 다했으니 후회는 없다. 굿럭 👍👍👍

## 결과

  - 12/15(수) 합격자 발표 👇👇
  - <img width="314" alt="스크린샷 2021-12-19 오후 1 04 17" src="https://user-images.githubusercontent.com/43846264/146663550-7cde5258-e96d-49f4-b5d5-aef6006eb1e8.png">



## 회고 (lessons learned)

  - CDSS 어드민 개발을 들어가기 전 어차피 한번은 훑어야 할 도메인 지식을 자격증 프레임에 맞춰 공부할 수 있어서 좋았음
  - 병원급에서 심사평가업무(예: 상병코딩 등)를 하고 있는 간호사, 조무사, 의무기록사 분들을 좀 만나보고 싶다는 생각이 듦
  - 심사평가사 준비과정에서 익힌 리서치 패턴(산정기준, 보험인정기준 등)은 지속적 빌드업 & 리마인드 필요
  - 보수교육 + 자격증 발급비용만 `70,000원`...장사는 이렇게 하는 것 (license 장사, 어떻게 기획하고 어떻게 시작할 수 있을까)
